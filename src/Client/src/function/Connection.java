...
public class Connection {
    ...
    public static void sendMessage(String text) throws Exception {
        ...
        ms.setID(Connection.getMyID());
        ms.setName(Connection.getMyName());
        ms.setMessage(text);
...
//        System.out.println(ms.getName().split("!")[0]);
        writeLog(Connection.getMyName(), file.getAbsolutePath());
        out.writeObject(ms);
        out.flush();
    }
    ...
    private static final String[] fileSizeUnits = {"bytes", "KB", "MB", "GB"};
    ...
    public static void setFileWriter(FileWriter fileWriter) {
        Connection.fileWriter = fileWriter;
    }
